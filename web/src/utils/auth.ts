 ...
 * 令牌存取(前端文档 4.1)。
 *
 * <p>两条硬规则,都是为了避开"看起来能用、偶发登出"的坑:
 * <ol>
 *   <li>**每次现读 localStorage**,不在模块变量里缓存。多标签页场景下别的标签会写新令牌,
 *       模块缓存会让本标签一直拿着过期值 —— 表现为"莫名其妙被登出",且无法复现</li>
 *   <li>写入必须是**同一时刻覆盖一对**:先写 token 再写 refreshToken 之间如果被打断,
 *       就会出现"新 token + 旧 refreshToken"的状态,下一次刷新必然撞上重放检测(后端会踢掉全部会话)</li>
 * </ol>
 */
